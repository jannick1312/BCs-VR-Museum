...
public partial class MediaPlacementController : Node
{
    ...
    private static readonly EventLogger Logger = new(nameof(MediaPlacementController));

    public override void _Ready()
    {
        var media2DInstance = GetNodeOrNull<Node3D>(Media2DInstancePath);
        var media2DPlaces = GetNodeOrNull(Media2DPlacesPath);
        var object3DInstance = GetNodeOrNull<Node3D>(Object3DInstancePath);
        var object3DPlaces = GetNodeOrNull(Object3DPlacesPath);
    ...
    public async Task Place(IReadOnlyList<DisplayMediaItem> items)
    {
        ...
        if (media2DItems.Count > 0)
            ...
            Logger.Info("Search result contains no images or videos to display.");

        if (object3DItems.Count > 0)
            await _object3DPlacement.Place(object3DItems);
        else
            Logger.Info("Search result contains no 3D objects to display.");
    }
