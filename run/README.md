...
| --- | --- | --- | --- |
| Android sideload<br>(`install.sh`/<br>`install.bat`) | The shared `config.json` is copied to the external Android path, creating or overwriting the destination file. | If the shared `media` directory exists, the scripts create the remote `images`, `videos`, and `3dPck` directories before copying the local directories. Files at the same relative path are overwritten and files that exist only on the headset remain untouched. | The installation scripts do not create or modify log files. |
...
4. If `run/media` exists, create the remote `images`, `videos`, and `3dPck` directories and copy each corresponding local directory that exists.
