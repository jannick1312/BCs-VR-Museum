...
During the development of this project, OpenAI Codex and ChatGPT were used with the GPT-5.5 and GPT-5.6 Sol models. The tools were used to support with technical questions, including comparing information from documentation and other sources, reviewing existing source code, and considering alternative implementations.

Codex was used especially during the preparation of the server backend, as setting up the backend was not a main focus of the bachelor thesis. All Python scripts located in [`tools/pipeline`](tools/pipeline/) were generated entirely with the help of these models. Since the pipeline consists of several scripts, Codex was prompted repeatedly until the intended and documented behavior was achieved. The generated code was not reviewed manually line by line. Instead, it was evaluated by executing the scripts and checking their outputs and behavior. The explanatory section [Processing details](tools/pipeline/README.md#processing-details) was also written with the assistance of Codex.

The [scripts](tools/vitrivr/scripts/) used to simplify starting and resetting the vitrivr server were also created with the assistance of Codex. Codex was given the individual steps required to start and reset the service. It then combined these steps into two scripts that execute the complete processes within a tmux session, allowing the session to be re-entered.

The `.bat` scripts located in [`run/deployment`](run/deployment/) were generated by Codex based on the corresponding Bash scripts in order to provide Windows support in addition to macOS support.

All code located in [`src`](src/) was written by the author. The code documentation was generated by Codex and reviewed by the author. Files in which Codex also assisted with parts of the implementation are marked at the bottom of the a file.

All hand-drawn-style technical pencil illustrations used in the project, including [vr-sketch.png](src/GodotMuseum/Menu/Settings/vr-sketch.png), [icon.png](src/GodotMuseum/Assets/Icon/icon.png), and the hand and controller sketches used in the [tutorial slides](src/GodotMuseum/Tutorial/Slides/), were generated with ChatGPT using the GPT-5.6 Sol model over multiple sessions and refined through several revisions. The general workflow involved giving a reference image, such as a specific hand pose, together with instructions to transform it into the desired pencil-sketch style.
